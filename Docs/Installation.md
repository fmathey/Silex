...
sources. A successful native build refreshes it even when the executable or
native objects are reused from cache. The most recent native `compile` or `run`
therefore defines the graph and target presented to native editor tooling.
